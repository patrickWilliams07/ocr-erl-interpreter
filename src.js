...
class ArrayCall extends Token{
    ...
    evaluate(ignoreLastIndex=false){
        ...
        for (let i = 0; i < this.indexes.length - (ignoreLastIndex ? 1 : 0); i++){ // converts ASTS to values
            let result = this.indexes[i].evaluate()
            if (result instanceof Error){
                return result
            }
            evaluatedIndexes.push(result)
        } // returns array of data types
        return this.callee.evaluate().get_index(evaluatedIndexes)
    }
...
class Length extends Property {
    evaluate(){
        let callee = this.callee.evaluate()
        if (callee instanceof Error){
            return callee
        }
...
class Substring extends Property {
    call(call){
        let string = this.callee.evaluate() // evaluates what it is a property of
        ...
        let index = call.argumentsAsts[0].evaluate() // first argument
        ...
        let length = call.argumentsAsts[1].evaluate() // second argument
...
class LeftOrRight extends Property {
    ...
    call(call){
        ...
        let length = call.argumentsAsts[0].evaluate() // first argument
        if (length instanceof Error){
            return length
        }
        if (!(length instanceof IntegerType)){
...
class UpperOrLower extends Property {
    ...
    evaluate(){
        let string = this.callee.evaluate()
...
class Open extends Subroutine {
    call(call){
        ...
        let fileName = call.argumentsAsts[0].evaluate()
...
class NewFile extends Subroutine {
    call(call){
        ...
        let fileName = call.argumentsAsts[0].evaluate() // filename is string value
        if (fileName instanceof Error){
            return fileName
        }
...
class ReadLine extends Property {
    call(call){
        let file = this.callee.evaluate() // gets file
...
class WriteLine extends Property {
    call(call){
        ...
        let toWrite = call.argumentsAsts[0].evaluate()
